...
void compare_arg(int argc, char *argv[], Stuff &command_var){
    ...
    try {
        ...
        if ((strcmp(argv[command_var.position], "-1")) == 0) {
            Matrix_ops<float> mat1 = open_file<Matrix_ops<float> >(argc, argv, command_var);
            std::cout << std::setprecision(2) << mat1.inv();
        }
...
T open_file(int argc, char *argv[],Stuff &command_var){
    ...
    if(!command_var.infile){
        std::cout << "\nError opening file: " << command_var.file1 << "\n\n";
...
void help(){
    ...
    std::cout << "\t= -out tells the current operator to be printed to a file =\n";
    std::cout << "\t= -add =\n";
    std::cout << "\t= -sub =\n";
    std::cout << "\t= -mul =\n";
    std::cout << "\t= -eq =\n";
